...
import Mutation, { IMutation } from '../models/Mutation';
...
const generatePropertyPDF = async (data: any, type: 'propertyTax' | 'mutation', id?: string): Promise<string> => {
    ...
    try {
      ...
      if (type === 'propertyTax') {
        ...
        if (data.statusTimeline && Array.isArray(data.statusTimeline)) {
          data.statusTimeline.forEach((step: any, index: number) => {
...
export const generatePropertyJPG = async (data: any, type: 'propertyTax' | 'mutation', id?: string): Promise<string> => {
  try {
    const fileName = `${type === 'propertyTax' ? 'property-tax-receipt' : 'mutation-status'}-${id || data.propertyId || data.applicationId}.jpg`;
    const jpgPath = path.join(UPLOAD_DIR, fileName);
...
export const getPropertyTax = async (req: Request, res: Response) => {
  try {
...
export const downloadPropertyTaxReceipt = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
export const getMutationStatus = async (req: Request, res: Response) => {
  try {
...
export const downloadMutationAcknowledgement = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { format } = req.query; // 'pdf' or 'jpg'
